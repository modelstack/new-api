<template>
    <div>
        ...
        <v-card variant="flat" class="mb-4 pa-2">
            ...
            <v-card-text class="pa-2 mt-2">
                <div class="chart-container">
                    <v-skeleton-loader v-if="loading && quotaData.length === 0" type="image" height="300" />
        ...
        <v-card variant="flat" class="pa-2">
            ...
            <v-card-text class="pa-2 mt-2">
                <v-data-table :headers="hourlyTableHeaders" :items="hourlyTableData" :loading="hourlyLoading"
        ...
        <!-- 系统公告时间线 -->
        <v-card variant="flat" class="mb-4 pa-2 mt-4">
            <v-card-title class="d-flex align-center">
                <v-icon class="mr-2" color="primary">mdi-bullhorn-variant</v-icon>
                系统公告
            </v-card-title>
            <v-card-text class="pa-2 mt-2">
                <!-- Loading State -->
                <div v-if="announcementLoading" class="d-flex flex-column align-center justify-center py-8">
                    <v-progress-circular indeterminate color="primary" size="32" />
                    <p class="text-medium-emphasis mt-3 text-body-2">加载中...</p>
                </div>

                <!-- Empty State -->
                <div v-else-if="processedAnnouncements.length === 0"
                    ...
                    <p class="text-body-2 text-medium-emphasis mt-3">暂无系统公告</p>
                </div>

                <!-- Timeline -->
                <v-timeline v-else align="start" density="compact" side="end" class="announcement-timeline">
                    <v-timeline-item v-for="(item, index) in processedAnnouncements" :key="index"
                        :dot-color="getTypeColor(item.type)" size="small">
                        <v-card class="announcement-card" variant="tonal" rounded="lg">
                            <v-card-text class="pa-4">
                                ...
                                <div v-if="item.htmlExtra" class="text-caption text-medium-emphasis mt-2 pt-2 border-t"
...
// 处理公告数据
const processedAnnouncements = computed<ProcessedAnnouncement[]>(() => {
    return (announcements.value || []).slice(0, 10).map((item) => {
        ...
        return {
            key: `${item?.publishDate || ''}-${(item?.content || '').slice(0, 30)}`,
...
async function refreshData() {
    ...
    await Promise.all([loadData(), loadHourlyData(), loadAnnouncements()])
