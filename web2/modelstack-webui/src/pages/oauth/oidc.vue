...
async function sendCode(code: string, state: string, retry = 0): Promise<void> {
  ...
  try {
    ...
    const { data: resData } = await API.get(`/api/oauth/oidc?code=${code}&state=${state}`)
    const { success, message: msg, data } = resData

    if (!success) {
      throw new Error(msg || 'OAuth2 授权失败')
    }
...
onMounted(async () => {
  ...
  if (savedState && state !== savedState) {
    showMessage('授权状态验证失败，请重试', 'error')
    message.value = '状态验证失败，正在跳转...'
    setTimeout(() => {
      router.push('/login')
    }, 2000)
    return
